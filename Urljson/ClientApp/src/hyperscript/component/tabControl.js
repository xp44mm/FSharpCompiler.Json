...
import { button, div, li, ul } from '../tags'
...
export function bindTabIndex(tabRoot, tabIndex = new BehaviorSubject(0)) {
    ...
    tabIndex.subscribe(i => {
        Array.from(navs.children).forEach((li, j) => {
        ...
        Array.from(panels.children).forEach((panel, j) => {
            if (i === j) {
    ...
    navs.pipeEvent('click', click$ =>
        click$
        |> map(e =>
            [...navs.children].map(e => e.firstChild).indexOf(e.target)
        )
        |> filter(i => i > -1)
